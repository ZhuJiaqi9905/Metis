...
class GPUCluster:
    def __init__(self, hostfile_path: str, clusterfile_path: str):
        self.host_entries: dict[int, dict[str, str | int]] = parse_hostfile(hostfile_path) # {id: {'ip': <str>, 'num_device': <int> } }
        self.nodes_info: dict[str, dict[str, str | int]] = parse_nodefile(clusterfile_path)

        self.nodes: dict[str, GPUNode] = dict() # { '${ip}': GPUNode}
    ...
    def get_total_num_devices(self) -> int:
        '''
        get the number of devices (GPUs) of all ranks 
        '''
    ...
    def get_device_memory_for_device_type(self, device_type: str) -> int:
        # print(f"node_info: {self.nodes_info}")
